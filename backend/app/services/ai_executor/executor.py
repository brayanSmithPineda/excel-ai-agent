...
class AICodeExecutor:
    ...
    async def execute_with_generated_code(
        ...
        try:
            execution_result = self.sandbox.execute_code(
                code=code,
                input_files=input_files or {}  # Use provided input files
            )
            
            # Step 4: Log to audit (if method exists)
            try:
                ...
                # Method doesn't exist yet, skip audit logging
                logger.warning("Audit logging method not available")
            
            # Step 5: Return results
            if execution_result.success:
                # Encode binary files as base64 strings for JSON serialization
                encoded_output_files = {}
                if execution_result.output_files:
                    import base64
                    for filename, file_bytes in execution_result.output_files.items():
                        encoded_output_files[filename] = base64.b64encode(file_bytes).decode('utf-8')
                
                return {
                    "success": True,
                    "output": execution_result.output,
                    "output_files": encoded_output_files,
                    "exit_code": execution_result.exit_code
                }
            else:
                return {
